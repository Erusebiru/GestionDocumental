...
<script>
    ...
    $(document).ready(function() {
        ...
        $(document).on('click','.pagination a',function(e) {
            ...
            getData('#usuarios',window.location.pathname + 'api/clientes',{'consulta':'','page':page,"_token": "{{ csrf_token() }}"});
        });

        $(document).on('click','.filtro',function(e){
            e.preventDefault();
            var consulta = $('[name="consulta"]').val();
            getData('#usuarios',window.location.pathname + 'api/clientes',{'consulta':consulta,'page':1,"_token": "{{ csrf_token() }}"});
        });
